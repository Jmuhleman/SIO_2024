...
public abstract class InsertionHeuristic implements ObservableTspConstructiveHeuristic {
    ...
    public final TspTour computeTour(TspData data, int startCityIndex, TspHeuristicObserver observer) {
        n = data.getNumberOfCities(); // Initialize n once
        ...
        if (tourList.size() > 2) {
            int lastCity = tourList.get(n - 1);
            int firstCity = tourList.getFirst(); // tourList.get(0);
            length += data.getDistance(lastCity, firstCity);
        }
    ...
    /**
     * Abstract method to update data structures after inserting a new city.
     *
     * @param data         The TSP data.
     * @param insertedCity The index of the city that has just been inserted.
     * @param inTour       Array indicating whether a city is already in the tour.
     */
    protected abstract void updateAfterInsertion(TspData data, int insertedCity, boolean[] inTour);
}
